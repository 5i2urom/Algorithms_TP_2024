...
{
    BTree<int> tree1(2); // Минимальная степень 2
    tree1.Insert(10);
    tree1.Insert(20);
    tree1.Insert(30);
    ...
    assert(tree1.root->keys[0] == tree2.root->keys[0]); // Проверка корневого узла
    assert(tree1.root->children[0]->keys[0] == tree2.root->children[0]->keys[0]); // Проверка дочернего узла

    // Тестирование оператора присваивания
    BTree<int> tree3(3); // Другое дерево с минимальной степенью 3
    tree3 = tree1;
    assert(tree1.root->keys[0] == tree3.root->keys[0]);
    assert(tree1.root->children[0]->keys[0] == tree3.root->children[0]->keys[0]);

    // Модификация копии не должна влиять на оригинал
    tree2.Insert(40);
    assert(tree1.root->keys.size() == 1); // tree1 не изменилось
    assert(tree2.root->keys.size() == 2); // tree2 изменилось

    std::cout << "Тесты конструктора копирования и оператора присваивания пройдены." << std::endl; 
}
...
int main()
{
    ...
    testCopyAndAssignment(); // Вызов функции с тестами
