...
bool HashTable<T, H1, H2>::Add( const T& key )
{
    ...
    for ( int i = 0; i < table.size(); ++i ) {
        ...
        if ( cell.State == Key && cell.Key == key )
            ...
            int insert_idx = hash;
            if ( firstDeleted >= 0 )
                insert_idx = firstDeleted;
            else
                --emptyCount;
            table[insert_idx] = HashTableCell( key, hash1, hash2, CellState::Key );
            ++keysCount;
            return true;
        }   
    }
...
void TestAssignmentOperator() {
    ...
    HashTable<std::string, StringHasher, SimpleStringHasher> originalTable(hasher, simpleHasher);
    originalTable.Add("apple");
    originalTable.Add("banana");
    originalTable.Add("cherry");

    HashTable<std::string, StringHasher, SimpleStringHasher> assignedTable(hasher, simpleHasher);
    assignedTable.Add("grape");
    assignedTable.Add("melon");

    assignedTable = originalTable;

    assert(assignedTable.Has("apple"));
    assert(assignedTable.Has("banana"));
    assert(assignedTable.Has("cherry"));

    // Проверка, что в assignedTable больше нет значений, которые были до присваивания
    assert(!assignedTable.Has("grape"));
    assert(!assignedTable.Has("melon"));
